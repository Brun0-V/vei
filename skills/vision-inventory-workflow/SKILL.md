...
- **MCP server** configured in your agent's MCP settings pointing to `vision_inventory_mcp.py` (see `configs/` directory).
...
1. **Run the deterministic vision pipeline:**
   ```
...
To configure the MCP server, copy the appropriate config from `configs/`, fill in the path to `vision_inventory_mcp.py` and your Cloudflare credentials.
